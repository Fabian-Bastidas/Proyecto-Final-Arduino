...
void loop(){
   ...
   if(nivelActual != 1 || digitalRead(ENTRADA_F) == LOW){
      muestraSecuencia();
      leeSecuencia();
   }
}
...
void leeSecuencia(){
   ...
   for(int i = 0; i < nivelActual; i++){
      flag = 0;
      while(flag == 0){
         if(digitalRead(ENTRADA_E) == LOW){
            digitalWrite(SALIDA_E, HIGH);
            tone(BUZZER, 600);
            delay(300);
            noTone(BUZZER);
            secuenciaUsuario[i] = SALIDA_E;
            flag = 1;
            delay(200);
